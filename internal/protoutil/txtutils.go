...
// ExtractConfigFromBlock extracts the config from a block
...
// GetEnvelopeFromBlock gets an envelope from a block's data
...
// FormSignedEnvelope creates a signed envelope from pre-existing signatures
func FormSignedEnvelope(
	...
	return FormSignedEnvelopeWithTLSBinding(txType, channelID, dataMsg, signatures, msgVersion, epoch, nil)
}

// FormSignedEnvelopeWithTLSBinding creates a signed envelope from pre-existing signatures with TLS binding
func FormSignedEnvelopeWithTLSBinding(
	...
	if err != nil {
		return nil, fmt.Errorf("error marshaling data message: %w", err)
	}

	// Create a payload without signature header since we'll use pre-existing signatures
	payload := &common.Payload{
		Header: &common.Header{
			ChannelHeader: MarshalOrPanic(payloadChannelHeader),
			// No SignatureHeader here as we're using pre-existing signatures
		},
		Data: data,
	}

	payloadBytes := MarshalOrPanic(payload)

	// Create envelope with the payload
	envelope := &common.Envelope{
		Payload: payloadBytes,
	}

	// If signatures are provided, use the first one as the envelope signature
	if len(signatures) > 0 {
		envelope.Signature = signatures[0]
	}
