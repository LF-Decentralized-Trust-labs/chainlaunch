...
func (c *deleteCmd) run(out io.Writer) error {
	...
	if !c.yes {
		fmt.Fprintf(out, "Are you sure you want to delete peer node with ID %d? [y/N]: ", c.id)
	...
	fmt.Fprintf(out, "Peer node with ID %d deleted successfully\n", c.id)
	return nil
}

// NewDeleteCmd returns the delete peer command
func NewDeleteCmd(logger *logger.Logger) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "delete",
		Short: "Delete a peer node",
		Long:  `Delete a Hyperledger Fabric peer node`,
